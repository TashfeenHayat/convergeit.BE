...
const getSendGridApiKey = (smtpConfig) => {
  const apiKey = smtpConfig?.authPass?.trim();
  if (!apiKey) {
    throw new Error('SendGrid API key missing: set authPass in SMTP config.');
  }
  return apiKey;
};

module.exports = { parseEmails, parseFrom, getSendGridApiKey, sendViaSendGridApi };
