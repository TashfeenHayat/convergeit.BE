...
const fs = require('fs');
const { parseEmails, sendViaSendGridApi } = require('../utils/sendGridApi');
...
const sendMail = async (smtpConfig, mailOptions) => {
    if (!smtpConfig?.authPass || smtpConfig?.authUser !== 'apikey') {
        throw new Error(
            'Report mail uses SendGrid HTTPS API only: set authUser to "apikey" and authPass to your SendGrid API key.'
        );
    }
